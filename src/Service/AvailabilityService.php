...
class AvailabilityService
{
    ...
    public function getAvailableDoctors(string $specialty, Date $date): array
    {
        ...
        $doctors = $this->staffTable->find()
            ...
            ->contain(['Services' => function ($q) {
                return $q->where(['Services.is_active' => true]);
            }])
            ->toArray();
     ...
     */
    private function getScheduleException(int $doctorId, Date $date)
    {
        if (!$this->scheduleExceptionsTable) {
            return null;
        }
